...
> **Plan Last Updated:** 2026-09-18 (Phase 2 completed — see §0)
...
- **(2026-09-18) Phase 2 completed on the Linux dev host, as far as it can go there — T-006, T-007, T-008 all done:**
  - T-006: `iosArm64` target reconfigured from a bare `iosArm64()` call to `iosArm64 { binaries.framework { baseName = "ComposeApp"; isStatic = true } }` in `composeApp/build.gradle.kts`, per D2/Q4.
  - T-007: `composeApp/src/iosMain/kotlin/com/example/rickandmorty/MainViewController.kt` added — `fun MainViewController() = ComposeUIViewController { App() }`, matching D3.
  - T-008: **discovered a host constraint the plan didn't call out** — `./gradlew :composeApp:linkDebugFrameworkIosArm64` runs but is unconditionally `SKIPPED` on Linux (`Skipping task ... as task onlyIf 'Task is enabled' is false`). Kotlin/Native can *compile* Apple targets to klib on any host (`compileKotlinIosArm64` succeeds on Linux, confirmed earlier), but the final **link** step that produces an actual `.framework`/binary requires Apple's linker (`ld64`) and is host-gated to macOS by the Kotlin Gradle plugin itself. This is a new, plan-relevant fact: **no framework-or-later verification (T-008 onward) is possible on the Linux host at all** — everything from here on must run inside the macOS VM.
  - T-008 completed for real in the macOS VM after cloning the repo there (T-010, ahead of its original slot — see below): `linkDebugFrameworkIosArm64` produced `composeApp/build/bin/iosArm64/debugFramework/ComposeApp.framework`; `Headers/ComposeApp.h` confirms `MainViewControllerKt` is exported correctly — `+ (UIViewController *)MainViewController __attribute__((swift_name("MainViewController()")));` — callable from Swift as `MainViewController()`, exactly per D3.
  - T-009 also completed early (done opportunistically alongside T-008, since both need the VM): VM's pre-installed Xcode is **16.2**, bundling the **iOS 18.2 SDK** — comfortably covers the iOS 18 device target, no Xcode update needed. Risk R-4's "Xcode may need updating" contingency did not materialize.
  - T-010 (independent `git clone` inside the VM) is therefore also done, pulled forward from Phase 3 because Phase 2's own verification (T-008) turned out to require the VM too.

**Not started yet:** Phase 3 (Xcode project `iosApp/` — T-010's clone is done, but T-011/T-012 aren't started), Phase 4 (signing, on-device run), Phase 5 (final cleanup docs — partially done ahead of schedule, since `CLAUDE.md` was already updated for the `jvmTest` move and the iOS target above, but not yet for Phase 2's completion or the Linux-can't-link finding).

**New, not anticipated by any risk in this plan:** the Linux-host link restriction (found at T-008) means Phase 1's Kotlin/Gradle work can keep happening on the Linux dev host, but **every task from T-008 onward is VM-only** — there's no way to dry-run framework/Xcode changes on Linux first. This tightens the dependency the plan already assumed (T-009/T-010 as Phase 3 prerequisites) into something stricter: they were effectively prerequisites for Phase 2's own exit criterion, not just Phase 3's.
...
### Phase 2: iOS Entry Point & UI Smoke Path (3-5 hours) — ✅ DONE (2026-09-18)
...
**Deliverable:** ~~`./gradlew :composeApp:linkDebugFrameworkIosArm64` (and `IosX64`)~~ **`./gradlew :composeApp:linkDebugFrameworkIosArm64`** produces `composeApp/build/bin/iosArm64/debugFramework/ComposeApp.framework` (or equivalent path) containing the expected public API (`MainViewControllerKt` visible from Swift). **Confirmed** — but only inside the macOS VM (see §0: the link task is unconditionally `SKIPPED` on the Linux host; framework linking is macOS-only).
**Prerequisites:** Phase 1 complete. ✅

### Phase 3: Xcode Project & VM Toolchain (5-8 hours — reduced from the original 8-14h now that Xcode is already installed; see Risk R-4) — IN PROGRESS (T-009/T-010 done early, see §0); **goal changed, see below**
...
| T-006 | ✅ DONE | ~~Configure `binaries.framework {}` for both iOS targets~~ Configured `binaries.framework { baseName = "ComposeApp"; isStatic = true }` for `iosArm64` only, per D2/Q4 | 2 | P0 | 1-2 hours | T-005 ✅ | KMP/Gradle |
| T-007 | ✅ DONE | Created `iosMain/.../MainViewController.kt` — `fun MainViewController() = ComposeUIViewController { App() }`, matching D3 | 2 | P0 | 1-2 hours | T-006 | Compose Multiplatform |
| T-008 | ✅ DONE (required the macOS VM — see §0) | ~~Verify framework link task produces a usable `.framework`~~ `./gradlew :composeApp:linkDebugFrameworkIosArm64` is unconditionally `SKIPPED` on the Linux host (linking Apple binaries is macOS-only, gated by `onlyIf` in the Kotlin Gradle plugin) — ran it in the VM instead; produced `ComposeApp.framework`, confirmed `Headers/ComposeApp.h` exports `MainViewControllerKt` correctly (`swift_name("MainViewController()")`) | 2 | P0 | 1 hour | T-007 | KMP/Gradle |
| T-009 | ✅ DONE (done early, alongside T-008) | Verify the VM's already-installed Xcode supports the iOS 18 SDK — Xcode + Command Line Tools are already present from the user's prior KMP attempt. Confirmed via `xcodebuild -version`/`xcodebuild -showsdks`: **Xcode 16.2**, bundling **iOS 18.2 SDK** — no update needed, R-4's update contingency did not materialize | 3 | P0 | 0.5-1 hour (verification only — see R-2 for the update-required contingency) | None (parallel with Phase 1-2) | macOS/VM admin |
| T-010 | ✅ DONE (done early, pulled forward from Phase 3 — see §0) | Set up VM ↔ repo access via `git clone` — independent `git clone` of this repository inside the VM (confirmed approach, §11 Q1); VM builds the Gradle project standalone, confirmed by T-008's successful link. Manual push/pull sync routine between VM clone and host working copy still to be exercised in practice during Phase 3-4 | 3 | P0 | 1-2 hours | T-009 | macOS/VM admin, Git |
...
- [x] ~~`composeApp/build.gradle.kts` declares `iosArm64()` and `iosX64()` targets~~ **`composeApp/build.gradle.kts` declares an `iosArm64()` target** (D1 revised — `iosX64()` dropped, no Gradle variant published) — `binaries.framework { baseName = "ComposeApp"; isStatic = true }` **configured** (T-001 done, T-006 done).
- [x] `iosMain` contains a Darwin Ktor engine dependency (T-003 done) — `MainViewController.kt` **created** (T-007 done); framework link verified in the macOS VM with `MainViewControllerKt` correctly exported (T-008 done — see §0 for the Linux-host link restriction discovered here).
