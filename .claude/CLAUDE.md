...
- There is no iOS app module/Xcode project yet — `iosArm64()` currently only gets the shared code to *compile* for iOS (`compileKotlinIosArm64`). The Xcode project, framework export config, and on-device run are later tasks in `ios-support-implementation-plan.md`.
...
- **Multiplatform build configuration / platform interop** — in progress. `composeApp` now has a working `iosArm64` target (`compileKotlinIosArm64` succeeds); the shared `Logger` gained an `iosMain` actual (`NSLog`-based). Still open: creating the actual Xcode project/app shell, exporting a framework (`binaries.framework {}` or CocoaPods), and running on the user's physical iPhone (no Mac hardware — only an Intel macOS VM — so no simulator target is usable; see "iOS target" above).
