...
class LocationViewModel(
    private val getLocationsUseCase: GetLocationsUseCase = Module.getLocationsUseCase
) {
    ...
    /**
     * A [kotlinx.coroutines.flow.StateFlow] that emits the current accumulated list of locations.
     */
    ...
    /**
     * Resets pagination state and fetches the first page of locations.
     */
    ...
    /**
     * Fetches the next page of locations and appends it to the current list.
    ...
    suspend fun loadNextPageIfNeeded() {
        ...
        try {
            getLocationsUseCase(nextPage).collect { lce ->
                when (lce) {
                    Lce.Loading -> Unit
                    is Lce.Content -> {
                        ...
                        _locations.value += lce.data.items
                    }
                    is Lce.Error -> {
                        awaitMinimumLoadingDuration(startMark)
                        _errorMessage.value = "The next page of locations could not be loaded."
                    }
                }
            }
        } finally {
