...
class CharacterViewModel(
    private val getCharactersUseCase: GetCharactersUseCase = Module.getCharactersUseCase
) {
    ...
    /**
     * A [kotlinx.coroutines.flow.StateFlow] that emits the current accumulated list of characters.
     */
    ...
    /**
     * Resets pagination state and fetches the first page of characters.
     */
    ...
    /**
     * Fetches the next page of characters and appends it to the current list.
    ...
    suspend fun loadNextPageIfNeeded() {
        ...
        try {
            getCharactersUseCase(nextPage).collect { lce ->
                when (lce) {
                    Lce.Loading -> Unit
                    is Lce.Content -> {
                        ...
                        _characters.value += lce.data.items
                    }
                    is Lce.Error -> {
                        awaitMinimumLoadingDuration(startMark)
                        _errorMessage.value = "The next page of characters could not be loaded."
                    }
                }
            }
        } finally {
